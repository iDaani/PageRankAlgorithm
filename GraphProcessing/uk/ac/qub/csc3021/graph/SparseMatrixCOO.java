...
public class SparseMatrixCOO extends SparseMatrix {
	...
	void readFile( BufferedReader rd ) throws Exception {
		...
		// ...
		destination_array = new int[num_edges];
		...
		for( int i=0; i < num_edges; ++i ) {
			...
			// ...
			source_array[i] = edge[0];
			destination_array[i] = edge[1];
		}
	}
	...
	public void edgemap( Relax relax ) {
		...
		// ...
		for (int i = 0; i < num_edges; i++) {
			int sourceValue = source_array[i];
			int destinationValue = destination_array[i];
			relax.relax(sourceValue, destinationValue);
		}
